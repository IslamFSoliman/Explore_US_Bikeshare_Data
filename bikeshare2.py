...
def time_stats(df):
    ...
    # Returning the most common/popular month conditioned if user input [All].
    common_month = df['Start_Month'].mode()[0]
    print(f"Most popular month is: {common_month}")

    # Returning the most common/popular day conditioned if user input [All].
...
def station_stats(df):
    ...
    most_frequent_start_end_stations = df['combination'].mode()[0]
    print(
         f"Most frequent start station and end stations are: {most_frequent_start_end_stations}"
         )
...
def trip_duration_stats(df):
    ...
    print(f"Average travel time in secounds is: {mean_travel_time}, \
        and in hours: {mean_travel_time/3600}")
...
def user_stats(df):
    ...
    if 'Birth Year' in df:
            ...
            print(f"Earliest birth year is: {earliest_byear}")

            recent_byear = int(df['Birth Year'].max())
            print(f"Most recent birth year is: {recent_byear}")

            common_byear = int(df['Birth Year'].mode()[0])
            print(f"Most common birth year is: {common_byear}")
